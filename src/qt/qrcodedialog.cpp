...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BGBT")
    updateDisplayUnit();
}
...
{
    QString ret = QString("BigBit:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BGBT unit input in lnReqAmount, we generate the URI with BGBT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BGBT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
